        ...
        <div class="col-md-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">Tipos de Archivos</h6>
                </div>
                <div class="card-body">
                <div class='row'>
                    <div class='col-12 d-flex justify-content-start'>
                        <button class='btn btn-primary' data-toggle="modal" onclick="btnmodal()"><i class='fas fa-plus-circle'></i>Ingresar Tipo de Archivo</button>
                    </div>
                </div>
                    <hr/>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addModalLabel">Ingresar Tipo de Archivo</h5>
            ...
            <form id="guardarcampo" action="{{ route('guardarta') }}" method="POST" enctype="multipart/form-data">
                    @csrf
...
$(document).ready(function() {
    ...
    function btnmodal() {
    $('#addModalLabel').html('Ingresar Tipo de Archivo');
...
var tablacampos = $('#campo-table').DataTable({
    ...
    columns: [
        ...
        { data: 'id', name: 'id', orderable: false, render: function(data, type) {
            return "<button onclick='sanciones_crud("+data+")' class='btn btn-danger btn-sm' title='Eliminar'><i class='fas fa-trash'></i></button><button class='btn btn-warning btn-sm edit-btn' onclick='actualizars("+data+")'><i class='fas fa-edit'></i></button>";
        }},
    ],
    order: [[3, 'desc']],
...
function sanciones_crud(id){
    if (confirm("¿Esta seguro de eliminar el registro?")) {
        $.ajax({
            ...
            cache: false,
            success: function(data){
                tablacampos.ajax.url('getCampota').load();
            }
        });
    } else {
        tablacampos.ajax.url('getCampota').load();
...
function actualizars(id) {
    $('#addModalLabel').html('Actualizar Tipo de Archivo');
    $('#per').hide();
    conid = id;
    $.ajax({
        url: 'mostrarta/' + id, 
