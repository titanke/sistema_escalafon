...
<x-layout-dato-legajo 
    ...
    :columns="['TRABJADOR','TIPO','FECHA CREACION','NRO FOLIO','ARCHIVO', 'Acciones']"
>
    @include('datoslegajo.forms.formOtrosArchivos')
</x-layout-dato-legajo>

@include('datoslegajo.modals.modalOtrosArchivos')
...
<script>
    ...
    document.getElementById('formAdd').addEventListener('submit', function(e) {
        e.preventDefault(); 
        onClickSaveAdd("archivos_adjuntos_crud","archivos_adjuntos_crud")
    });
    
    document.getElementById('formEdit').addEventListener('submit', function(e) {
        e.preventDefault();
        onClickSaveEdit("archivos_adjuntos_crud", "archivos_adjuntos_crud");
    });
    ...
    var dataTable = $('#dataTable').DataTable({
        ...
        columns: [
            ...
            {title: 'TIPO', data: 'tipo_archivo', name: 'tipo_archivo' },
            ...
            { 
                title: "ACCIONES",  data: 'id', name: 'id', orderable: false, render: function(data, type) {
                    ...
                    <div class="d-flex flex-row">
                        <button onclick='viewCRUD(${data},"archivos_adjuntos_crud","Archivo")' id="btnVer" class="btn btn-info mr-2">
                            <i class="fas fa-eye" id="editIcon"></i>
                        </button>
                        @hasanyrole('ADMIN|COORDINADOR')
                        <button onclick='deleteCRUD(${data},"borrarot","archivos_adjuntos_crud")' class='btn btn-danger btn-sm mr-2' title='Eliminar'>
                            <i class='fas fa-trash'></i>
                        </button>
                        <button onclick='editCRUD(${data},"archivos_adjuntos_crud","Archivo")' class='btn btn-info' title='Editar'>
