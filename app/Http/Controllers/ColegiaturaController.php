...
class ColegiaturaController extends Controller
{
    ...
    public function index(Request $request) {
        $columns = Schema::getColumnListing('colegiatura');
        $columns = array_diff($columns, ['created_at','updated_at']);
            $prefixedColumns = array_map(function ($column) {
            return "colegiatura.$column";
        }, $columns);

        $df = DB::table('colegiatura')
            ->leftJoin('archivo', 'colegiatura.archivo', '=', 'archivo.id')
            ->leftJoin('personal', 'colegiatura.personal_id', '=', 'personal.id_personal')
            ...
            if ($request->has('filter_field') && $request->has('filter_value')) {
                ...
                if (!empty($filterValue) && in_array($filterField, $columns)) {
                    $df->where("colegiatura.$filterField", $filterValue);
                }
            }
            $df = $df->get();
