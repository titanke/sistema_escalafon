...
class TiempoServicioController extends Controller
{
    ...
    public function index(Request $request) {
        $columns = Schema::getColumnListing('tiempo_servicio');
        $columns = array_diff($columns, ['created_at','updated_at']);
            $prefixedColumns = array_map(function ($column) {
            return "tiempo_servicio.$column";
        }, $columns);

        $df = DB::table('tiempo_servicio')
            ->leftJoin('tipodoc', 'tiempo_servicio.idtd', '=', 'tipodoc.id')
            ->leftJoin('archivo', 'tiempo_servicio.archivo', '=', 'archivo.id')
            ->leftJoin('personal', 'tiempo_servicio.personal_id', '=', 'personal.id_personal')
            ...
            if ($request->has('filter_field') && $request->has('filter_value')) {
                ...
                if (!empty($filterValue) && in_array($filterField, $columns)) {
                    $df->where("tiempo_servicio.$filterField", $filterValue);
                }
            }
            $df = $df->get();
    ...
    public static function store(Request $request)
    {
        ...
        if ($archivo) {
            ...
            $estudio = TiempoServicio::create($data);
            return response()->json($estudio->toArray());
        } else {
            $data = $request->except('archivo');
            TiempoServicio::create($data);
        }
    ...
    {
        $t = TiempoServicio::find($id);
        $data = $request->except('archivo');

        if ($t) {
            //ASIGNACION TRATADO COMO COMPENSACION 
